...
# ============================================================================
# CRITICAL: Patch triton.ops BEFORE any other imports
# This prevents: ModuleNotFoundError: No module named 'triton.ops'
# ============================================================================
import types
...
def _patch_triton_modules():
    ...
    # First, try to import the real triton module
    try:
        import triton as real_triton
        # If triton is already imported, just ensure triton.ops exists
    ...
    # If triton is not installed, create mock modules
    ...
    if 'triton' not in sys.modules:
        ...
        sys.modules['triton'].ops = triton_ops
        if not hasattr(sys.modules['triton'], '__spec__'):
            sys.modules['triton'].__spec__ = importlib.util.spec_from_loader('triton', loader=None)

    # Create triton.runtime module
    triton_runtime = types.ModuleType('triton.runtime')
    triton_runtime.__spec__ = importlib.util.spec_from_loader('triton.runtime', loader=None)
    triton_runtime.driver = types.ModuleType('triton.runtime.driver')
    triton_runtime.driver.__spec__ = importlib.util.spec_from_loader('triton.runtime.driver', loader=None)
    sys.modules['triton.runtime'] = triton_runtime
    sys.modules['triton.runtime.driver'] = triton_runtime.driver

    # Create triton.testing module
    triton_testing = types.ModuleType('triton.testing')
    triton_testing.__spec__ = importlib.util.spec_from_loader('triton.testing', loader=None)
    triton_testing.get_dram_gbps = lambda: 900
    triton_testing.get_max_simd_tflops = lambda: 1000
    triton_testing.get_max_tensorcore_tflops = lambda: 1000
    triton_testing.nvsmi = lambda: {}
    sys.modules['triton.testing'] = triton_testing

    # Create triton.language module
    triton_language = types.ModuleType('triton.language')
    triton_language.__spec__ = importlib.util.spec_from_loader('triton.language', loader=None)
    sys.modules['triton.language'] = triton_language

    # Create triton.cdiv function
    def cdiv(a, b):
        return (a + b - 1) // b
    sys.modules['triton'].cdiv = cdiv

# Call the patch function
_patch_triton_modules()

# Additional patch: Ensure triton.__spec__ is set correctly
try:
    import triton
    ...
    print(f"Warning: Could not patch triton.__spec__: {e}")
...
# Patch for seen_tokens compatibility with Transformers library
# This fixes: AttributeError: 'LinearAttentionTKWindowCache' object has no attribute 'seen_tokens'
try:
    from model.linear_attention.linear_attention import LinearAttentionState
    if not hasattr(LinearAttentionState, 'seen_tokens'):
        @property
        def _seen_tokens_property(self):
            return self._seen_tokens
        LinearAttentionState.seen_tokens = _seen_tokens_property
except Exception as e:
    print(f"Warning: Could not patch LinearAttentionState: {e}")
...
def main():
    ...
    if not args.no_init_eval:  # Check on summary example
        ...
        try:
            ...
            print('Continuing without initial evaluation...')
    ...
    if 'save10' not in args.distill_config and 'save10' not in args.finetune_config:
        ...
        try:
            ...
            # Save the model state even if evaluation fails
            checkpoint_path = join(args.checkpoint_dir, f'{args.run_name}_emergency_checkpoint.pt')
