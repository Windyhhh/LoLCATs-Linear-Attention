...
class LolcatsLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        # Use new signature for transformers >= 4.38
        ...
        # Handle different transformers versions for rotary_emb
        if hasattr(self, 'rotary_emb'):
            position_embeddings = self.rotary_emb(hidden_states, position_ids)
        else:
            # For newer versions, rotary_emb might be in a different location
            # Try to get it from the first layer
            position_embeddings = self.layers[0].self_attn.rotary_emb(hidden_states, position_ids)
