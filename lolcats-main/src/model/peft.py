...
def create_peft_config(model: Module, 
    ...
    if peft_config['method'] == 'lora':
        # Workaround for triton.ops import error in bitsandbytes
        import sys
        import importlib
        import types

        # CRITICAL: Patch triton.ops BEFORE any imports
        ...
        # Also create triton module if needed
        if 'triton' not in sys.modules:
            triton = types.ModuleType('triton')
            triton.ops = sys.modules['triton.ops']
            sys.modules['triton'] = triton

        # Also patch bitsandbytes to skip triton_based_modules
        try:
            import bitsandbytes.nn
            # Monkey-patch to skip triton_based_modules import
            if not hasattr(bitsandbytes.nn, 'StandardLinear'):
                bitsandbytes.nn.StandardLinear = None
                bitsandbytes.nn.SwitchBackLinear = None
                bitsandbytes.nn.SwitchBackLinearGlobal = None
                bitsandbytes.nn.SwitchBackLinearVectorwise = None
        except Exception as e:
            print(f"Warning: Could not patch bitsandbytes.nn: {e}")
